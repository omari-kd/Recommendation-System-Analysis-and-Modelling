...
# Load libraries
# ----------------------------
# data.table: for fast and memory-efficient data manipulation on large datasets
library(data.table)

# dplyr: for data manipulation using a grammar of data manipulation (e.g. filtering, mutating, summarising)
library(dplyr)

# tidyr: for tidying and reshaping data (e.g. pivoting data from long to wide format)
library(tidyr)

# ggplot2: for creating elegant and complex data visualisations
library(ggplot2)

# caret: for predictive modelling and machine learning workflows, including pre-processing and dummy variable creation
library(caret)

# janitor: for cleaning data and column names to ensure consistent and tidy data frames
library(janitor)

# reshape2: for data reshaping operations (e.g. melt and cast), useful if tidyr functions are insufficient
library(reshape2)

# scales: for scaling and formatting axes in ggplot2 visualisations (e.g. percentage labels)
library(scales)

# lubridate: for easy manipulation of dates and times
library(lubridate)

# isotree: for implementing Isolation Forests, an anomaly detection algorithm
library(isotree)

# Matrix: for working with sparse matrices efficiently, useful in high-dimensional data scenarios
library(Matrix)

# randomForest: for building Random Forest models for classification or regression tasks
...
ggplot(bot_users, aes(x = anomaly_score, fill = factor(final_bot_flag))) +
...
**Rename the merged_df to final_df**
...
**Handling missing values in the final_df to final_dt**
...
```{r}
save(rf_model, file = "cart_prediction_model.RData")
