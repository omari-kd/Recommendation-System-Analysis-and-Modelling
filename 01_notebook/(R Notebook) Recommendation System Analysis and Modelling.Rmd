...
> By computing the time difference between consecutive events, one can observe if some visitors have extremely short intervals between actions which is a common bot behaviours.
>
> **\
...
iso_model <- isolation.forest(features, ntree = 100, sample_size = nrow(features), seed = 42)

# Get predictions which include an 'anomaly_score' for each user.
...
# Add the anomaly score to the visitor_stats data.table.
visitor_stats[, anomaly_score := predictions$anomaly_score]

# Flag a user as anomalous if their anomaly_score is in the top 1% of scores.
anomaly_threshold <- quantile(predictions$anomaly_score, 0.99, na.rm = TRUE)
visitor_stats[, anomaly_flag := ifelse(predictions$anomaly_score > anomaly_threshold, -1, 1)]
...
# --------------------------------------
# Filter Out Bot Users from Events Data
# -------------------------------------
...
**Data Proprocessing for item_properties**
